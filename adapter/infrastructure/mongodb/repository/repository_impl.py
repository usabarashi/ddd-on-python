...
class RepositoryImpl(repository.Repository):
    ...
    async def get(
        ...
        got_employee = None if got_employee_dto is None else employee.Employee(
            ...
            duties=ImmutableSequence([governance.Duties(item)
                                      for item in got_employee_dto.duties]),
        ...
        pprint(got_application_dto.route)
        got_application = None if got_application_dto is None else application.Application(
            ...
            route=application.Route([application.Progress(
        ...
        got_workflow = None if got_workflow_dto is None else workflow.Workflow(
    ...
    async def save(
        ...
        if application_entity is not None:
            ...
            application_document.update(
                {"_id": application_document.pop("id_")})
